from flask import Flask, request, render_template, redirect, url_for, send_file, session, Response, jsonify
...
def validate_class_section(form, field):
    pattern = r"^(VI|VII|VIII|IX|X|XI|XII)-[A-E]$"
...
def validate_admission_number_login(form, field):
...
class AuthorizeForm(FlaskForm):
    admission_number = StringField('Admission Number', validators=[InputRequired(), Length(min=4, max=4, message="Please enter a valid 4-digit admission number")])
    passcode = PasswordField('Passcode', validators=[InputRequired(), Length(min=6, max=6, message="Please enter a valid 6-digit passcode")])
    submit = SubmitField('Login')

class LoginForm(FlaskForm):
    admission_number = StringField('Admission Number', validators=[InputRequired(), validate_admission_number_login])
    ...
    email_id = StringField('Email ID', validators=[InputRequired(), Email(message="Please enter a valid Email ID")])
    submit = SubmitField('Login')
